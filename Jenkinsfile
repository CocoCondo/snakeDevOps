pipeline {
  ...
  parameters {
    choice(name: 'APP', choices: ['notes', 'snake'], description: 'Qué app desplegar')
  ...
  environment {
    REGISTRY = 'docker.io'
    REGISTRY_NAMESPACE = 'tu_usuario_docker'          // TODO
    IMAGE = "${env.REGISTRY}/${env.REGISTRY_NAMESPACE}/${params.APP}:${params.VERSION}"
    DOCKER_CREDENTIALS_ID = 'dockerhub-creds'         // TODO (Jenkins > Credentials)
    KUBECONFIG_CREDENTIALS_ID = 'kubeconfig-minikube' // TODO (Jenkins > Credentials, tipo "Secret file")
    TF_WORKDIR = 'infra'                               // carpeta donde están tus .tf (si aplica)
  }
